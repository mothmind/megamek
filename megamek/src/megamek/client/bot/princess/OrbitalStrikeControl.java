...
public class OrbitalStrikeControl {
    ...
    public Optional<Coords> selectTarget(Princess owner, OrbitalSupport support) {
        ...
        Coords best = null;
        double bestValue = damage * MINIMUM_VALUE_FRACTION;

        for (Coords candidate : candidates) {
            double value = scoreHex(candidate, owner, damage, boardId);
            if (value > bestValue) {
                bestValue = value;
                best = candidate;
            }
     ...
     */
    private double scoreHex(Coords center, Princess owner, int damage, int boardId) {
        ...
        for (int distance = 0; distance <= OrbitalSupport.BLAST_RADIUS; distance++) {
            ...
            for (Coords coords : ring) {
                value += scoreSingleHex(damageAtDistance, coords, owner, boardId);
            }
        }
     ...
     */
    private double scoreSingleHex(int damage, Coords coords, Princess owner, int boardId) {
